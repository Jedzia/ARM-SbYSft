...
hex: $(BINARY).hex
srec: $(BINARY).srec
srecs: $(BINARY).srec $(BINARY).S
assembly: $(BINARY).S
list: $(BINARY).list
GENERATED_BINARIES=$(BINARY).elf $(BINARY).bin $(BINARY).hex $(BINARY).srec $(BINARY).list $(BINARY).map $(BINARY).S
...
%.S: %.c
	@#printf "  CC      $(*).c\n"
	$(Q)$(CC) $(TGT_CFLAGS) $(CFLAGS) $(TGT_CPPFLAGS) $(CPPFLAGS) -S -o $(*).S -c $(*).c
